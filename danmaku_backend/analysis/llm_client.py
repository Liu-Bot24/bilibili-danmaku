...
from danmaku_backend.services.llm_usage import record_llm_usage
...
class LLMClient:
    ...
    def _chat_json_with_config(
        ...
        *,
        ...
        max_tokens: int | None,
        result_validator: Callable[[dict[str, Any]], bool] | None,
    ) -> dict[str, Any] | None:
        payload = {
            "model": config["model"],
            "messages": messages,
            ...
            try:
                ...
                if parsed is not None:
                    if result_validator is not None:
                        try:
                            valid_result = result_validator(parsed)
                        except Exception:
                            valid_result = False
                        if not valid_result:
                            log_error("文本分析服务返回结果不完整")
                            return None
                    return parsed
