...
class ReportStore:
    def __init__(
        ...
        self.persistent_dir = Path(persistent_dir) if persistent_dir is not None else self.report_dir / "persistent"
    ...
    def ensure(self) -> None:
        ...
        os.chmod(self.persistent_dir, 0o755)
    ...
    def save_report(
        ...
        *,
        ...
        comment_analysis: dict[str, Any] | None = None,
        ...
        comment_payload = self._validate_analysis_block(comment_analysis, "comment_analysis")
        full_payload = self._validate_analysis_block(full_analysis, "full_analysis")
        if not content_payload and not deep_payload and not comment_payload and not full_payload:
        ...
        report = {
            ...
            "comment_analysis": comment_payload,
            "full_analysis": full_payload,
        }
    ...
    def get_report(self, report_id: str) -> dict[str, Any] | None:
        ...
        if report:
            if not self._is_expired(report):
                return report
            self._archive_report(path, report)
        return self._read_persistent_report(report_id)
    ...
    def _read_persistent_report(self, report_id: str) -> dict[str, Any] | None:
        report = self._read_json_file(self._persistent_report_path(report_id))
        if not report:
            return None
        stored_id = self._normalize_report_id(report.get("report_id"), required=True)
        return report if stored_id == report_id else None
    ...
    def _persistent_report_path(self, report_id: str) -> Path:
        return self.persistent_dir / f"{report_id}.json"
